...
import freerobux.get.rbx.counter.rblx.calcpro.rbuxcalc.meme.MemeAdapter
import freerobux.get.rbx.counter.rblx.calcpro.rbuxcalc.meme.MemeListItem
import freerobux.get.rbx.counter.rblx.calcpro.rbuxcalc.meme.MemeRepository
...
class MemeActivity : ChromeFlowActivity() {
    ...
    private fun buildItems(): List<MemeListItem> {
        val out = mutableListOf<MemeListItem>()
        MemeRepository.memes.forEachIndexed { index, meme ->
            out += MemeListItem.MemeRow(meme)
            val isEndOfGroup = (index + 1) % 3 == 0
            if (isEndOfGroup) out += MemeListItem.BannerRow(AdAssets.pickBannerSpec())
        }
        return out
    }
